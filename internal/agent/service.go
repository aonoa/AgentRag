...
func (s *Service) Ask(ctx context.Context, req domain.AskRequest) (domain.AskResponse, error) {
	...
	if len(plannedSubQueries) > s.cfg.PlannerMaxSubqueries {
		plannedSubQueries = plannedSubQueries[:s.cfg.PlannerMaxSubqueries]
	}
	ctxOrch, cancel := context.WithTimeout(ctx, time.Duration(s.cfg.OrchestratorTimeoutSeconds)*time.Second)
	defer cancel()
	stats := &orchestrationStats{SubQueries: len(plannedSubQueries), StartAt: time.Now()}
	...
	for attempt := 1; attempt <= s.cfg.MaxRetries; attempt++ {
		...
		retrievalResult, refRows, err := s.retrieveForPlannedSubQueries(ctxOrch, route, rewritten, plannedSubQueries, req.ExtraFilters, stats)
		if err != nil {
			return domain.AskResponse{}, err
		}
		refs = refRows
		...
		out, err := s.runnable.Invoke(ctx, workflowInput{
			Question: rewritten,
			Route:    route,
			Context:  contextText,
			Attempt:  attempt,
		...
		grade, err := s.grade(ctx, rewritten, out.Answer, contextText)
		if err != nil {
			return domain.AskResponse{}, err
		}
		...
		debug[fmt.Sprintf("attempt_%d", attempt)] = map[string]any{
			"rewritten": rewritten,
			"grade":     grade,
			"hits":      len(retrievalResult.Candidates),
...
func (s *Service) AskStream(ctx context.Context, req domain.AskRequest) (AskStreamResult, error) {
	...
	debug := map[string]any{"route": route}
	for attempt := 1; attempt <= s.cfg.MaxRetries; attempt++ {
		rewritten, retrievalResult, refs, contextText, err := s.prepareAttempt(ctxOrch, route, current, attempt, req.ExtraFilters, plannedSubQueries, stats)
		if err != nil {
			return AskStreamResult{}, err
		}

		system, user := s.finalAnswerPrompts(rewritten, route, contextText)
		...
		if attempt < s.cfg.MaxRetries {
			...
			grade, err := s.grade(ctx, rewritten, probe, contextText)
			if err != nil {
				return AskStreamResult{}, err
			}
			debug[fmt.Sprintf("attempt_%d", attempt)] = map[string]any{
...
func (s *Service) buildWorkflow(ctx context.Context) (compose.Runnable[workflowInput, workflowOutput], error) {
	...
	routeNode := compose.InvokableLambda(func(_ context.Context, in workflowInput) (workflowInput, error) {
		return in, nil
	})
	answerNode := compose.InvokableLambda(func(ctx context.Context, in workflowInput) (workflowOutput, error) {
		prompt := "你是企业知识库问答助手。优先使用上下文回答，回答要简洁准确，并在信息不足时明确说明。"
		user := fmt.Sprintf("问题：%s\n\n路由：%s\n\n上下文：\n%s\n\n请给出最终答案。", in.Question, in.Route, in.Context)
		ans, err := s.llm.Chat(ctx, s.cfg.LLMModel, prompt, user)
		if err != nil {
			return workflowOutput{}, err
		}
		msg := schema.AssistantMessage(ans, nil)
		return workflowOutput{Answer: strings.TrimSpace(msg.Content)}, nil
	})

	wf.AddLambdaNode("route", routeNode).AddInput(compose.START)
	wf.AddLambdaNode("answer", answerNode).AddInput("route")
	wf.AddEnd("answer")
...
func (s *Service) route(ctx context.Context, question string) (routeDecision, error) {
	...
	switch val {
	case domain.RouteDirectChunk, domain.RouteHierarchical, domain.RouteSQL, domain.RouteWeb, domain.RouteDirect, domain.RouteHybrid:
